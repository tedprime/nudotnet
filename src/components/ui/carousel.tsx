...
const CustomProjectCarouselNext = React.forwardRef<
  HTMLButtonElement,
  React.ComponentProps<typeof Button>
>(({ variant = 'outline', size = 'icon', ...props }, ref) => {
  const { scrollNext, canScrollNext } = useCarousel();
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className="absolute right-[12%] top-1/2 grid aspect-square h-14 w-14 -translate-y-1/2 place-items-center rounded-full bg-white text-sm font-medium transition-all hover:scale-105 hover:bg-white focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 min-[1440px]:right-3 min-[1520px]:-right-3"
      disabled={!canScrollNext}
      onClick={scrollNext}
      {...props}
    >
...
CustomProjectCarouselNext.displayName = 'CarouselNext';
